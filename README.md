...
- metadata_partition: The specific metadata column used to partition the genomic profiles. For example, this column might refer to the outbreak number and the contain such entries as "1", "2", etc. If an individual sample row in the sample sheet is missing the `metadata_partition` entry, that sample will not be included in the analysis. If `--ignore_empty_metadata_partition` is set to `false` it will instead be reported in the `arborator/metadata.excluded.tsv` output file.
...
More information about parameters can be found in the [docs/usage.md](docs/usage.md) documentation.
...
- `--ar_tree_distances`: Instructs genomic address service to interpret distance matrices distances as either cophenetic (the distance at which two clusters or leaves cluster together) or patristic (the sum of branch lengths between clusters or leaves) (default: cophenetic).
- `--ar_sort_matrix`: Whether or not to sort the samples by sample name in the distance matrix generated by the Genomic Address Service software (default: true).
- `--ar_max_cpus`: Modifys the arborator parameter:`--n_threads`. `--n_threads` indicates numbers of threads to use with multithreading for arborator. (default: 4).
